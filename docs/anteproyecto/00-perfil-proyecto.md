...
**Condiciones del sector.** Ese crecimiento ocurre en una economía marcadamente informal: el **empleo informal alcanzó el 86,8 % de la población ocupada en 2024** —6,0 de 6,9 millones de personas—, según el Cuadro 7 de UDAPE, elaborado con la Encuesta Continua de Empleo del INE. El indicador se emplea como caracterización cualitativa y no interviene en ningún cálculo del documento (capítulo 2, sección 2.1.1). Para el rubro de talleres esto se traduce en unidades de negocio pequeñas, con presupuesto de tecnología muy limitado y gestión apoyada todavía en papel u hojas de cálculo.
...
| **Límites y exclusiones** | Enumerados en la sección 7.4 |
...
2. ¿Qué modelo de datos, qué políticas de seguridad a nivel de fila y qué contrato de interfaz permiten representar la jerarquía de organizaciones y talleres sosteniendo un único límite de aislamiento?
...
| 2 | **Diseñar** el modelo de datos de la jerarquía de organizaciones y talleres, las políticas de seguridad a nivel de fila y el contrato de la interfaz de programación que sostienen un único límite de aislamiento entre organizaciones | Especificación de requerimientos, modelo entidad-relación, políticas, diagramas C4, contrato de la interfaz |
...
| 4 | **Validar** el aislamiento mediante pruebas automatizadas por la interfaz de programación y por acceso directo a la base de datos, contrastando la arquitectura propuesta con la línea base y comprobando que la separación se sostiene con la verificación de membresía deshabilitada | Suite de aislamiento con matriz de requisito, caso y evidencia; resultados de todas las condiciones experimentales en tres corridas |
...
Su descarte no afecta el objetivo central ni el cierre del proyecto.

---

## 5. Variables

El **criterio de éxito** de la validación está fijado de antemano en la sección 9.4.

| Tipo | Variable | Indicador | Instrumento / escala | Obj. |
|---|---|---|---|---|
| **Independiente** | Arquitectura de aislamiento | Condición experimental: línea base, arquitectura completa, sin verificación de aplicación y acceso directo al motor | Banco de pruebas / nominal | 4 |
...
El despliegue serverless, con escalado a cero y sin costo fijo por organización, reduce el costo de infraestructura a lo que se consume y cabe en la capa gratuita de sus proveedores (sección 12), condición para ofrecer software especializado a un sector con **86,8 %** de empleo informal (sección 2). Administrar varias organizaciones desde una sola cuenta elimina, además, la duplicación de cuentas y registros por local.
...
| **Diseño** | **Cuasiexperimental con línea base, sobre caso único**: el aislamiento resuelto solo en la aplicación frente a la arquitectura completa, a la arquitectura sin verificación de aplicación y al acceso directo al motor. En el objetivo 5, intrasujeto contrabalanceado: selector de contexto frente a cambio de cuenta |
...
La configuración detallada de cada instrumento está en el [anteproyecto](04-anteproyecto-integrado.md), sección 15.

| Obj. | Técnica | Instrumento | Frecuencia | Producto |
|---|---|---|---|---|
| 1 | Revisión sistemática de literatura | Cadena booleana y matriz autor, metodología, aporte, limitaciones | Una revisión con lectura título, resumen y conclusiones | Estado del arte y vacío |
...
**Criterio de éxito.** El objetivo 4 se declara cumplido solo si, en las **tres** corridas: (1) la línea base devuelve filas ajenas —si no fuga, el resultado se declara **no concluyente**—; (2) con la arquitectura propuesta se devuelven **cero filas ajenas** en las siete tablas; y (3) con la arquitectura completa el **100 %** de las operaciones sobre datos ajenos responde con el error especificado. Un caso omitido por falta de entorno **no** se contabiliza como cumplido.
...
| **Esp. 4:** ¿En qué medida se reducen las filas ajenas frente a la línea base, también sin la verificación de membresía? | Validar frente a la línea base | Criterio de éxito (sección 9.4) | VI: condición experimental; VD: aislamiento | Cuasiexperimental, censo, 3 corridas / 0 filas ajenas; 100 % autorización correcta |
| **Esp. 5:** ¿Es el cambio de contexto más eficiente y satisfactorio que el cambio de cuenta? | Evaluar la usabilidad *(complementario)* | Éxito ≥ 80 % por tarea y SUS ≥ 68 con el selector | VI: modelo de cambio de contexto; VD: usabilidad | Intrasujeto, n = 30 / éxito ≥ 80 %; SUS ≥ 68; t pareada p < 0,05 |
...
| **F4 — Validación y cierre** | 8–21 de diciembre | 4, 5 y 6 | Evidencia de todas las condiciones experimentales, evaluaciones complementarias, documento final y defensa |
...
| **I8** | 8–21 dic | Validación en todas las condiciones experimentales, sesiones con operadores, medición de costo, redacción final y defensa |
...
| Diagnóstico | X | | | | | | | |
| Diseño | | X | | | | | | |
| Esquema, políticas y autenticación | | | X | | | | | |
| Organizaciones, talleres y miembros | | | | X | | | | |
| Clientes | | | | | X | | | |
| Inventario y reclutamiento | | | | | | X | | |
| Cliente web y publicación | | | | | | | X | |
| Validación y cierre | | | | | | | | X |
...
| **H5 — Validación concluida** | 21 de diciembre | todas las condiciones experimentales en tres corridas; objetivos complementarios ejecutados o descartados; documento final |
...
El registro completo, con plan de contingencia, está en el [Plan de trabajo](../ingenieria/08-plan-trabajo.md), sección 6.
...
| Evidencia reproducible de aislamiento | 0 filas ajenas con la arquitectura propuesta en 3 corridas, frente a filas ajenas en la línea base |
