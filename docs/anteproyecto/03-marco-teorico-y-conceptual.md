...
El fundamento de esa medición es, además, lo que sostiene una afirmación central del proyecto: que la arquitectura sea correcta no implica que sea utilizable, y una jerarquía de dos niveles que el operador no logra manejar no resuelve el problema que la motivó (capítulo 1, sección 1.2).
...
**Design Science Research.** Hevner et al. (2004) establecen que en la investigación en sistemas de información la contribución puede ser un **artefacto** —un constructo, un modelo, un método o una instancia—, siempre que se diseñe para resolver un problema relevante y se **evalúe con rigor**. Es el marco que legitima que el resultado de este proyecto sea un sistema construido y no solo un informe: el diseño responde al vacío del capítulo 2, sección 2.3, el desarrollo produce la instancia evaluable (objetivo específico 3) y la evaluación es la validación cuasiexperimental frente a la línea base (objetivo específico 4). La exigencia de rigor es la que obliga a fijar de antemano el criterio de decisión y a contrastar la arquitectura con una alternativa, en lugar de limitarse a mostrar que funciona.
...
Ninguna de las teorías anteriores se acepta como verdad absoluta. Para cada una se consigna su limitación documentada y la adaptación que impone el contexto real del proyecto: una pequeña organización boliviana, con presupuesto de tecnología reducido (capítulo 2, sección 2.1) y un único desarrollador.
...
| **Microservicios** (Newman, 2021) | Despliegue y escalado independientes por servicio | El propio autor advierte que introducen complejidad operativa —observabilidad distribuida, consistencia entre servicios, sobrecarga de despliegue— y desaconseja adoptarlos sin una organización que los sostenga | Se descarta. El comparable más cercano del estado del arte (capítulo 2, sección 2.2) los emplea, pero un desarrollador único no puede sostener esa carga operativa. Se adopta una aplicación modular desplegada como funciones, que conserva la separación lógica sin el costo distribuido |
...
| **Design Science Research** (Hevner et al., 2004) | Legitimar un artefacto como contribución de investigación | Exige una evaluación rigurosa: un artefacto que solo se demuestra funcionando no constituye evidencia | La evaluación se ancla en una **línea base** y en un criterio de decisión fijado antes de ejecutar (sección 13.4 del anteproyecto) |

**Sobre la viabilidad financiera en el contexto boliviano.** La literatura de arquitectura suele asumir condiciones —despliegue multirregión, equipos de plataforma, presupuesto de observabilidad— que no se sostienen en un sector con 86,8 % de empleo informal y talleres de presupuesto mínimo (capítulo 2, sección 2.1). Criticar la teoría, en este proyecto, consiste en retener el principio y descartar la implantación: se conserva el aislamiento verificable, la verificación explícita y la entrega automatizada, y se descartan la distribución en microservicios, la redundancia multirregión y la infraestructura de confianza cero completa. Toda decisión de esta clase queda registrada como ADR con su justificación económica explícita.
...
| Antecedente (capítulo 2, sección 2.1) | Problema (capítulo 1, sección 1.2) | Teoría que lo sustenta (sección 3.2) | Artefacto del proyecto |
|---|---|---|---|
| Parque de motocicletas en crecimiento sostenido; operadores que abren más de un local | Pérdida de la visión consolidada del cliente entre talleres | Modelos de multi-tenancy y niveles de aislamiento (Krebs et al., 2012) | Jerarquía de organizaciones y talleres con alcance de datos por nivel (ADR-006) |
...
|---|---|
| Arquitectura de microservicios en profundidad | Se menciona solo como alternativa descartada (sección 3.3); desarrollarla no ayuda a resolver el problema de aislamiento |
...
| Teoría de colas y modelado de rendimiento | Las pruebas de carga están excluidas del alcance (capítulo 1, sección 1.8.3); modelar rendimiento no tendría contraparte verificable |
...
| Interoperabilidad con facturación electrónica del SIN | Excluida del alcance (capítulo 1, sección 1.8.3); pertenece al trabajo futuro y su marco normativo se citaría sin uso en este documento |
...
*Autor corporativo, sin fecha de publicación fija. Se emplean **solo como sustento del marco conceptual** (sección 3.1), nunca como sustento teórico. Fecha de recuperación consignada: 14 de agosto de 2026.*
...
> **Nota sobre E8 (Chong et al., 2006).** El artículo original de MSDN fue retirado por Microsoft. Se cita la copia archivada, comprobada y accesible. Por tratarse de documentación corporativa **no se emplea como sustento teórico**: la taxonomía de modelos de multi-tenancy se atribuye en la sección 3.2.1 a Krebs et al. (2012), que es literatura revisada por pares.
