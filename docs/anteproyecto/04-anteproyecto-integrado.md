...
1. Título del proyecto, 2. Planteamiento del problema, 3. Preguntas de investigación, 4. Justificación, 5. Antecedentes, 6. Estado del arte, 7. Marco teórico y conceptual, 8. Objetivo general, 9. Objetivos específicos, 10. Variables, 11. Operacionalización, 12. Matriz de consistencia, 13. Enfoque y tipo, 14. Población y muestra, 15. Técnicas e instrumentos, 16. Estrategia de análisis, 17. Consideraciones éticas

**Parte II — Propuesta técnica**

18. Antecedentes tecnológicos relevantes, 19. Propuesta de solución tecnológica, 20. Arquitectura preliminar del sistema, 21. Tecnologías y herramientas, 22. Diseño técnico preliminar, 23. Estrategia de implementación, 24. Cronograma de trabajo, 25. Resultados esperados, 26. Viabilidad técnica, 27. Referencias
...
**Efectos.** El historial del cliente queda fragmentado entre talleres de una misma organización —la gestión de varias organizaciones desde una cuenta está ausente en las 10 plataformas relevadas (sección 14.2)—; un solo error de consulta expone datos de una organización a otra, riesgo que la evidencia de fallos recurrentes en la aplicación de políticas de seguridad de fila vuelve concreto (sección 6.4); el operador que crece lleva cada local como una cuenta independiente o recurre a hojas de cálculo; y el costo de entrada excluye a buena parte de un sector con 86,8 % de empleo informal (sección 5.1).
...
**Pregunta general.** La formulación del problema (sección 2.3) constituye la pregunta general. Se estructura como tecnología + efecto esperado + magnitud + condición + contexto, contrastada con la alternativa del mercado, y es directamente resoluble construyendo el sistema y midiendo sobre él.
...
2. ¿Qué modelo de datos, qué políticas de seguridad a nivel de fila y qué contrato de interfaz permiten representar la jerarquía de organizaciones y talleres sosteniendo un único límite de aislamiento? *(Diseño)*
...
El despliegue serverless, con escalado a cero y sin costo fijo por organización, reduce el costo de infraestructura a lo que se consume: la especificación del proyecto cabe en la capa gratuita de sus proveedores (sección 26.3), condición necesaria para ofrecer software especializado a un sector con **86,8 %** de empleo informal (sección 5.1). Administrar varias organizaciones y talleres desde una sola cuenta elimina, además, la duplicación de cuentas y registros que hoy impone el software de un solo inquilino. El objetivo complementario 6 cuantifica ese costo por organización.
...
| **SaaS de un solo inquilino por cuenta** | La oferta relevada con presencia en Bolivia y en la región (sección 14.2) | Traslada el sistema a la nube pero conserva el supuesto de un taller por cuenta; el aislamiento entre cuentas, cuando existe, vive en el código |
...
| **Esquema compartido con políticas en el motor** | Políticas de seguridad a nivel de fila evaluadas por el propio motor de base de datos (PostgreSQL Global Development Group, s. f.-b) | Traslada la condición de inquilino al motor, pero la literatura la estudia sobre inquilinos **planos** y documenta fallos del propio mecanismo (sección 6) |
...
| **Búsqueda** | Cadena booleana `("multi-tenant" OR "multi-tenancy" OR "multitenancy") AND ("row-level security" OR "tenant isolation" OR "data isolation") AND ("SaaS" OR "shared schema" OR "cloud")`, con términos del *ACM Computing Classification System* —*Security and privacy, subcategoría Database and storage security*; *Software and its engineering, subcategoría Software architectures*—, en ACM Digital Library, IEEE Xplore, Scopus, Google Scholar, BASE y OATD |
| **Selección** | Lectura progresiva título, resumen y conclusiones. Inclusión: revisión por pares o tesis de posgrado en ciencias de la computación; problema arquitectónico comparable aunque el rubro difiera. Exclusión: soluciones sobre tecnologías legadas y artículos de opinión sin validación métrica |
| **Síntesis** | Análisis **crítico**: de cada trabajo se consigna dónde falla frente al problema, y esa columna alimenta el enunciado del vacío (sección 6.5) |
...
El presente proyecto aborda esta deficiencia mediante el **diseño, desarrollo y validación de una arquitectura multi-tenant jerárquica (organización y talleres)** que mantiene un **único límite de aislamiento verificable** a nivel de organización, tratando el taller como criterio de alcance operativo; refuerza la seguridad a nivel de fila con **verificación de membresía en la capa de aplicación**; y **contrasta la separación obtenida con una línea base** de aislamiento solo en la aplicación, por dos vías independientes.
...
| **Design Science Research** (Hevner et al., 2004) | Exige que la evaluación sea rigurosa, no una demostración | La evaluación se ancla en una línea base y en un criterio de éxito fijado de antemano |

### 7.4 Alineación metodológica

| Antecedente (sección 5) | Problema (sección 2) | Teoría (sección 7.2) | Objetivo, artefacto |
|---|---|---|---|
| Operadores que abren más de un local | Historial del cliente fragmentado | Multi-tenancy (Krebs et al., 2012) | O2 — jerarquía de organizaciones y talleres con alcance por nivel |
...
Sintetiza **qué** se construye, **para quién** y **con qué impacto**, y está ligado al título y al problema central (sección 2.1).
...
Cuatro objetivos secuenciales —**Diagnosticar, Diseñar, Desarrollar y Validar**—, acotados a lo mínimo necesario para responder la pregunta general:
...
| 2 | **Diseñar** el modelo de datos de la jerarquía de organizaciones y talleres, las políticas de seguridad a nivel de fila y el contrato de la interfaz de programación que sostienen un único límite de aislamiento entre organizaciones | Especificación de requerimientos, modelo entidad-relación con alcance por nivel, políticas y funciones de verificación, diagramas C4, contrato de la interfaz |
...
| 4 | **Validar** el aislamiento mediante pruebas automatizadas por la interfaz de programación y por acceso directo a la base de datos, contrastando la arquitectura propuesta con la línea base y comprobando que la separación se sostiene con la verificación de membresía deshabilitada | Suite de aislamiento con matriz de requisito, caso y evidencia; resultados de la línea base y de la arquitectura propuesta en tres corridas reproducibles |
...
| 6 | **Evaluar** el costo operativo mensual por organización del despliegue serverless frente a un despliegue en servidor dedicado | Métricas de consumo suficientes para imputar costo por organización | 7 de diciembre de 2026 | El costo queda como análisis de viabilidad económica (sección 26.4) |

Su descarte no afecta el objetivo central ni el criterio de cierre del proyecto.
...
|---|---|---|---|---|---|
| Arquitectura de aislamiento | VI (cualitativa) | Capas activas | Condición experimental: línea base, arquitectura completa, sin verificación de aplicación y acceso directo al motor | Banco de pruebas / nominal | 4 |
...
| **Esp. 4:** ¿En qué medida la arquitectura reduce las filas ajenas frente a la línea base, y se sostiene sin la verificación de membresía? | Validar el aislamiento frente a la línea base, con la verificación deshabilitada | Cero filas ajenas con la arquitectura propuesta, incluso con la verificación de aplicación deshabilitada | VI: condición experimental, de la línea base al acceso directo al motor; VD: aislamiento, gestión centralizada | Cuasiexperimental, censo de 7 tablas y todas las operaciones, 3 corridas / filas ajenas (0); autorización correcta (100 %); casos en verde sin la aplicación (100 %) |
| **Esp. 5** *(complementaria)*: ¿Es el cambio de contexto más eficiente y satisfactorio que el cambio de cuenta? | Evaluar la usabilidad del cambio de contexto frente al cambio de cuenta | Éxito ≥ 80 % por tarea y SUS ≥ 68 con el selector | VI: modelo de cambio de contexto; VD: usabilidad | Intrasujeto contrabalanceado, n = 30 / éxito ≥ 80 %; SUS ≥ 68 con α > 0,8; t de Student pareada, p < 0,05 |
...
## 13. Enfoque y tipo de investigación
...
| **Alcance** | **Descriptivo, propositivo y explicativo** | Descriptivo en el diagnóstico, propositivo en el diseño y el desarrollo, y explicativo en la validación, donde se contrasta la arquitectura con la línea base |
| **Método** | **Experimental-comparativo** | El criterio de éxito se fija antes de la validación y se somete a pruebas capaces de refutarlo |

---

### 13.1 Tipo de diseño

**Cuasiexperimental con línea base, sobre caso único.** Se contrasta el **antes** —el aislamiento resuelto solo en la capa de aplicación, que es lo que hace hoy la oferta relevada— con el **después** —la arquitectura propuesta, con las políticas del motor activas—, sobre el mismo escenario y con la misma suite de pruebas.

El proyecto **no formula hipótesis**: al no proponerse una magnitud de mejora, sino la verificación de una propiedad, la validación se gobierna con un **criterio de éxito** fijado de antemano, declarado antes de ejecutar y sometido a pruebas capaces de refutarlo (sección 13.4).

### 13.2 Condiciones de la validación

| Condición | Políticas del motor | Verificación de membresía | Vía | Resultado esperado |
|---|---|---|---|---|
| **Línea base** | Deshabilitadas | Omitida | Interfaz y base de datos | Se devuelven filas ajenas |
| **Arquitectura completa** | Activas | Activa | Interfaz | Cero datos ajenos; error de autorización según el contrato |
| **Sin verificación de aplicación** | Activas | Sustituida por una versión que concede sin comprobar | Interfaz | Cero filas ajenas |
| **Acceso directo al motor** | Activas | No interviene | Base de datos | Cero filas ajenas |

La **línea base** se establece únicamente en el proyecto de base de datos dedicado y desechable, con datos sintéticos: se deshabilita la seguridad a nivel de fila en las siete tablas de negocio y se sustituye la verificación de membresía, de modo que el sistema quede con un único control, el de la aplicación. Nunca se aplica en *staging* ni en producción.

La tercera condición es la que sostiene la **inmutabilidad del aislamiento**: si el aislamiento se mantiene con el control de la aplicación anulado, la separación no depende de esa capa.

### 13.3 Escenario de validación

Lo construye la propia prueba, nunca datos preexistentes.

| Cuenta | Organizaciones, talleres y datos que administra |
|---|---|
| **Cuenta A** | Organización 1, con los talleres 1.1 y 1.2, cada uno con sus repuestos, y los clientes de la organización. Organización 2, distinta, bajo la misma cuenta |
| **Cuenta B** | Organización 3, con el taller 3.1 y sus clientes propios |
| **Cuenta C** | Sin membresía en ninguna de las anteriores |

Un solo montaje cubre las tres preguntas del aislamiento: **entre cuentas** (A frente a B), **entre organizaciones de una misma cuenta** (1 frente a 2) y **frente a quien no es miembro de ninguna** (C).

### 13.4 Secuencia y criterio de éxito
...
2. Ejecución en la **línea base**, con registro de las filas ajenas devueltas.
3. Restauración de las políticas y ejecución en las otras tres condiciones, una por vez.
4. Repetición del ciclo completo **tres veces**, en momentos distintos y sobre entornos reconstruidos, para descartar un resultado dependiente de una ejecución.
5. Contraste contra el criterio de éxito y conservación de la evidencia.

**Criterio de éxito.** El objetivo 4 se declara cumplido solo si, en las **tres** corridas:

1. **La línea base discrimina**: devuelve filas ajenas en las siete tablas de negocio. Si no las devuelve, el montaje no distingue entre arquitecturas y el resultado se declara **no concluyente**, nunca confirmatorio.
2. Con la arquitectura propuesta se devuelven **cero filas ajenas** en las siete tablas, también con la verificación de la aplicación anulada y por acceso directo al motor.
3. El **100 %** de las operaciones del contrato sobre datos ajenos responde con el error de autorización especificado.

Un solo caso en contrario impide declararlo cumplido, y se reporta con su tabla y su caso.

## 14. Población y muestra

### 14.1 Población documental *(objetivo 1)*

| Elemento | Definición |
|---|---|
| **Población** | Publicaciones revisadas por pares sobre aislamiento entre inquilinos en esquemas compartidos, 2021–2026, indexadas en las bases de la sección 6.1 |
| **Muestra** | **10 fuentes** seleccionadas |
| **Muestreo** | No probabilístico **por criterio**, con los criterios de inclusión y exclusión de la sección 6.1 |

### 14.2 Población de soluciones del mercado *(objetivo 1)*
...
### 14.3 Población técnica *(objetivos 2 y 4)*
...
### 14.4 Población de operadores *(objetivo 5)*
...
### 14.5 Consumo de infraestructura *(objetivo 6)*
...
| **Aislamiento — vía interfaz** | Pruebas de contrato | Vitest con cliente HTTP | Estado y código `modulo.razon` por operación | Totalidad de las operaciones en la línea base, la arquitectura completa y la arquitectura sin verificación de aplicación, en 3 corridas |
| **Independencia de capas** | Sustitución de la verificación de membresía | Vitest en el banco de pruebas | Casos en verde | Arquitectura sin verificación de aplicación, en 3 corridas |
...
| **Costo** *(obj. 6)* | Telemetría del proveedor | Paneles de consumo de Supabase y Vercel, tarifas publicadas | Invocaciones, transferencia, tamaño de base, convertidos a USD | Una lectura diaria durante los 14 días de I8 |

### 15.2 Recolección de los datos de aislamiento
...
| **2 — Construir el escenario** | La propia prueba crea las cuentas, organizaciones y talleres del escenario de validación, con correos irrepetibles sobre un dominio reservado | Escenario reproducible |
| **3 — Ejecutar** | Se corre la suite en cada condición, una por vez: línea base, arquitectura completa, sin verificación de aplicación y acceso directo al motor | Resultado binario por caso, código de respuesta y recuento de filas |
...
### 15.3 Recolección de los datos de usabilidad y de costo
...
### 15.4 Validez y confiabilidad de los instrumentos
...
## 16. Estrategia de análisis

### 16.1 Aislamiento *(objetivo 4)*

**Descriptivo y por criterio.** Recuento de filas ajenas por tabla y condición; porcentaje de tablas con políticas activas; porcentaje de operaciones con respuesta correcta; comparación de la línea base frente a la arquitectura propuesta. La decisión se toma con el criterio de conjunción de las condiciones evaluadas.
...
### 16.3 Costo y calidad *(objetivos 6 y 3)*
...
### 16.4 Representación y herramientas

Tabla de filas ajenas por tabla y condición; gráfico de la línea base frente a la arquitectura propuesta; barras de éxito por tarea y condición con la línea del umbral; diagrama de caja de tiempos por condición; distribución de SUS frente al baremo. El procesamiento se realiza con **hoja de cálculo** sobre la salida exportada de la suite y la planilla de sesiones, y los contrastes con **R** (funciones base `t.test`, `shapiro.test` y `wilcox.test`).

---

## 17. Consideraciones éticas

### 17.1 Privacidad por diseño
...
### 17.2 Protección de los entornos

Las pruebas **no se ejecutan contra ningún entorno productivo ni contra infraestructura de terceros**. El escenario de línea base, con las políticas deshabilitadas, se aplica **únicamente** en el proyecto de validación dedicado y desechable, con datos sintéticos, y nunca en *staging* ni en producción. Las pruebas de carga, estrés y penetración están excluidas (sección 2.2), de modo que el procedimiento no puede degradar el servicio de nadie. La credencial privilegiada se lee **únicamente del entorno**, nunca del repositorio (RNF-103).

### 17.3 Participación de personas
...
- Si **una sola** tabla devolviera filas ajenas con la arquitectura propuesta, se reporta la fuga con su tabla y su caso, y el criterio de éxito no se cumple.
...
## 18. Antecedentes tecnológicos relevantes

La propuesta integra cuatro tecnologías consolidadas, cuya evolución se resume en la sección 5.2:
...
## 19. Propuesta de solución tecnológica

### 19.1 Problema e impacto

Se atiende el problema central de la sección 2.1: la ausencia de un sistema que centralice varias organizaciones y talleres sin exponer sus datos. Su relevancia la sostienen el crecimiento del parque de motocicletas (sección 5.1), la ausencia de gestión multiorganización en las 10 plataformas relevadas y la evidencia de fallos del aislamiento en producción (sección 6.4).

### 19.2 Solución e innovación
...
La tecnología seleccionada es oportuna porque sus componentes están maduros y disponibles en capa gratuita (sección 26.3), y factible porque la desarrolla un solo desarrollador en un ecosistema único —TypeScript— sobre servicios gestionados que no exigen operación de servidores.

### 19.4 Análisis comparativo
...
### 19.5 Alcance funcional y técnico
...
### 19.7 Matriz investigación–solución

| Objetivo | Módulo de software que lo materializa |
|---|---|
| 1 — Diagnosticar | — (insumo de diseño: fija la línea base que reproduce la línea base) |
...
| 4 — Validar | Suite de aislamiento por las dos vías y banco de pruebas de las condiciones experimentales |
...
## 20. Arquitectura preliminar del sistema

### 20.1 Estilo arquitectónico
...
### 20.2 Diagrama de contenedores (C4, nivel 2)
...
### 22.2 Requerimientos, reglas y seguridad
...
### 23.3 Estrategia multinivel de pruebas
...
| Aislamiento | Dada la identidad de otra cuenta, cuando consulta directamente las 7 tablas con la arquitectura propuesta, entonces obtiene cero filas ajenas | Aislamiento | 0 filas ajenas en 3 corridas | Pendiente |
...
### 24.1 Descomposición del trabajo (EDT)
...
| **6 — Validación** | Suite de aislamiento en todas las condiciones, pruebas extremo a extremo y auditorías, evaluación de usabilidad, medición de costo |

**Ruta crítica**: especificación y contrato; esquema y políticas; autenticación y contexto activo; organizaciones, talleres y miembros; clientes e inventario; cliente web y, por último, validación. El contrato de la interfaz se cierra **antes** de construir el cliente web. La estimación se hace en puntos de historia.

### 24.2 Fases e iteraciones
...
| **F4 — Validación y cierre** *(obj. 4, 5 y 6)* | I8 | 8–21 dic | Todas las condiciones experimentales en tres corridas, sesiones con operadores, medición de costo, documento final |

*Reserva: del 22 al 31 de diciembre, para correcciones posteriores a la revisión del tutor.*

### 24.3 Hitos y puntos de decisión
...
| **H5 — Validación concluida** | 21 de diciembre | Todas las condiciones experimentales en tres corridas, con evidencia conservada; objetivos 5 y 6 ejecutados o descartados con su criterio; documento final entregado |
...
### 24.4 Diagrama de Gantt

| Actividad | S1 | S2 | O1 | O2 | N1 | N2 | D1 | D2 |
|---|:-:|:-:|:-:|:-:|:-:|:-:|:-:|:-:|
| Diagnóstico | X | | | | | | | |
| Diseño y especificación | | X | | | | | | |
| Esquema, políticas, autenticación y contexto | | | X | | | | | |
| Organizaciones, talleres y miembros | | | | X | | | | |
| Clientes | | | | | X | | | |
| Inventario, reclutamiento de operadores | | | | | | X | | |
| Cliente web y publicación | | | | | | | X | |
| Validación, evaluaciones y cierre | | | | | | | | X |

---

## 25. Resultados esperados

### 25.1 Resultados técnicos
...
| Evidencia reproducible de aislamiento | 0 filas ajenas con la arquitectura propuesta en 3 corridas |
...
### 25.2 Impacto frente a la línea base
...
### 26.1 Restricciones y supuestos
...
| Proyecto de validación | Supabase, proyecto desechable | Mismas especificaciones; ocupa el lugar de *staging* durante I8 | Condiciones experimentales | USD 0 |
...
| **Operativa** | Servicios gestionados sin servidores que operar; publicación automática tras el pipeline; registros de ejecución del proveedor y del motor de datos para el diagnóstico, y paneles de consumo para el costo ([Arquitectura](../ingenieria/04-arquitectura.md), sección 15) |
...
|---|---|---|---|
| 1 | Problema | La evidencia empírica | Sección 2.1, a partir de las secciones 5.1 y 6.4 y del relevamiento de la sección 14.2 |
| 2 | Objetivos | El problema | Secciones 8 y 9, a partir del problema central y las causas raíz de la sección 2.1 |
| 3 | Solución propuesta | Los objetivos | Sección 19 y matriz investigación–solución de la sección 19.7 |
| 4 | Requerimientos | La solución y su alcance | Sección 22, a partir de la sección 19.5 |
| 5 | Arquitectura | Los requerimientos, sobre todo los no funcionales | Sección 20, a partir de RNF-101, RNF-102, RNF-301 y RNF-302 |
| 6 | Tecnologías | La arquitectura elegida | Sección 21, a partir del estilo de la sección 20.1 |
| 7 | Metodología de desarrollo | El alcance y el tiempo | Secciones 23.1 y 23.2, a partir de las secciones 19.5 y 24 |
| 8 | Validación | Los criterios de aceptación | Secciones 13.4, 16 y 23.6, a partir del criterio de éxito |
| 9 | Planificación | Todo lo anterior | Secciones 24 y 26, a partir de la EDT de la sección 24.1 |

| Qué se sustenta | Con qué se sustenta |
|---|---|
| El alcance | Límites y exclusiones (secciones 2.2 y 19.6) y viabilidad (sección 26.4) |
