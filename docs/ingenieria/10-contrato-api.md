...
|---|---|---|
| **Ausencia de estado** | Estilo REST (marco teórico §3.2.1) | Cada petición porta todo lo necesario para ser atendida. El servidor no conserva contexto entre llamadas, ni siquiera la empresa sobre la que se opera. |
| **Contexto explícito** | ADR-005 | La empresa y la sucursal activas viajan en cabeceras. El servidor **nunca** asume un valor por defecto. |
...
|---|---|---|
| `X-Org-Id` | Identificador de la **empresa activa** | En toda operación sobre datos internos de una empresa |
| `X-Workshop-Id` | Identificador de la **sucursal activa** | En toda operación sobre entidades de **nivel sucursal** (inventario y sus movimientos) |

Reglas de validación, en este orden:

1. Si falta la cabecera exigida, la petición se rechaza —no se elige una empresa ni una sucursal por defecto (ADR-005, RF-303).
2. `X-Org-Id` se valida contra la **membresía activa** del solicitante. Sin membresía activa no hay acceso (RF-701).
3. `X-Workshop-Id` se valida como **perteneciente a la empresa activa**. Una sucursal de otra empresa se trata como inexistente (RF-303, RNF-105).
...
| Falta `X-Workshop-Id` en una operación de nivel sucursal | `400` | `workshop.missing_active_workshop` |
| Sin membresía activa en la empresa indicada | `403` | `organization.access_denied` |
| Membresía activa, pero el rol no habilita la operación | `403` | `<modulo>.insufficient_permissions` |
| La sucursal indicada no pertenece a la empresa activa | `404` | `workshop.not_found` |

### 2.3 Regla de rutas

El identificador de la empresa aparece en la ruta **solo cuando el recurso es la empresa misma** dentro de la colección de la cuenta: `/api/organizations`, `/api/organizations/{orgId}` y su cambio de contexto. Todo recurso **interior** a una empresa —sucursales, miembros, clientes, inventario, auditoría— se dirige sin identificadores de contexto en la ruta y se resuelve por cabecera.

Es la aplicación literal de ADR-005: se descartó anidar la empresa en la ruta de cada recurso (`/api/empresas/{id}/clientes`) porque acopla la estructura de rutas a la jerarquía. Admitir la ruta anidada para unos recursos y la cabecera para otros dejaría **dos** mecanismos de contexto conviviendo, y la validación dejaría de estar concentrada en un punto único.

### 2.4 Representación

- Los cuerpos de petición y de respuesta son JSON.
- Los campos usan la misma grafía que el modelo de datos (`snake_case`), de modo que el contrato y el esquema hablen un solo vocabulario.
...
Las transiciones de estado no se modelan como edición de un campo: la baja lógica de una sucursal o de un cliente es una acción con consecuencias de auditoría (RF-703), y el contrato la distingue de un `PATCH` ordinario.
...
En las tablas siguientes, la columna **Contexto** indica qué cabeceras son obligatorias además de la credencial: `—` ninguna, `Org` requiere `X-Org-Id`, `Org+Suc` requiere además `X-Workshop-Id`. La columna **Rol** indica el rol mínimo; `Miembro` significa cualquier rol con membresía activa.

### 3.1 Identidad y cuenta

| Método | Ruta | Contexto | Rol | Requisito | Resultado |
|---|---|---|---|---|---|
| `POST` | `/api/auth/register` | — | *(sin credencial)* | RF-101 | Crea la cuenta, su primera empresa, su primera sucursal y la membresía `owner`, **en un solo acto** (ADR-007) |
| `GET` | `/api/auth/me` | — | *(cuenta autenticada)* | RF-104, RF-202 | Perfil de la cuenta y lista de empresas donde tiene membresía activa, con el rol en cada una |
...
| Método | Ruta | Contexto | Rol | Requisito | Resultado |
|---|---|---|---|---|---|
| `GET` | `/api/organizations` | — | *(cuenta autenticada)* | RF-202 | Empresas de la cuenta **según membresía activa**, no según quién las creó |
| `POST` | `/api/organizations` | — | *(cuenta autenticada)* | RF-201 | Crea una empresa; el solicitante queda como `owner` |
| `GET` | `/api/organizations/{orgId}` | — | Miembro | RF-202 | Datos de la empresa |
| `PATCH` | `/api/organizations/{orgId}` | — | Owner | RF-204 | Modifica los datos de la empresa |
| `POST` | `/api/organizations/{orgId}/switch` | — | Miembro | RF-203 | Confirma la empresa como activa y devuelve el rol del solicitante en ella |

`switch` no cambia estado en el servidor —no hay sesión que actualizar (§1)—: **valida** que la cuenta pueda operar sobre esa empresa y devuelve el rol, para que el cliente guarde el contexto y lo envíe en las llamadas siguientes.

### 3.3 Sucursales

| Método | Ruta | Contexto | Rol | Requisito | Resultado |
|---|---|---|---|---|---|
| `GET` | `/api/workshops` | Org | Miembro | RF-302 | Sucursales de la empresa activa |
| `POST` | `/api/workshops` | Org | Owner | RF-301 | Crea una sucursal |
| `GET` | `/api/workshops/{workshopId}` | Org | Miembro | RF-302 | Datos de la sucursal |
| `PATCH` | `/api/workshops/{workshopId}` | Org | Owner | RF-301 | Modifica los datos de la sucursal |
...
| `GET` | `/api/workshops/{workshopId}/assignments` | Org | Miembro | RF-304 | Miembros asignados a la sucursal |
| `POST` | `/api/workshops/{workshopId}/assignments` | Org | Owner | RF-304 | Asigna un miembro a la sucursal |
| `DELETE` | `/api/workshops/{workshopId}/assignments/{userId}` | Org | Owner | RF-304 | Retira la asignación, sin afectar la membresía |

La asignación a sucursales es **operativa**: no otorga ni restringe permisos, y no filtra lo que el miembro puede ver (ADR-006, decisión cerrada en [07-decisiones-diseno.md](07-decisiones-diseno.md)).

### 3.4 Miembros

| Método | Ruta | Contexto | Rol | Requisito | Resultado |
|---|---|---|---|---|---|
| `GET` | `/api/members` | Org | Miembro | RF-407 | Miembros de la empresa activa, con rol y estado |
| `POST` | `/api/members/invite` | Org | Owner | RF-401, RF-402, RF-406 | Incorpora a una cuenta **existente** con rol `mechanic` o `receptionist` |
...
- El propietario de la empresa no puede ser removido ni ver su rol modificado.
...
### 3.5 Clientes — nivel empresa

| Método | Ruta | Contexto | Rol | Requisito | Resultado |
|---|---|---|---|---|---|
| `GET` | `/api/clients` | Org | Miembro | RF-502, RF-504 | Clientes de la empresa activa, **con independencia de la sucursal activa** |
| `POST` | `/api/clients` | Org | Owner, Receptionist | RF-501, RF-503, RF-505 | Registra un cliente en la empresa activa |
...
Que estas operaciones **no** exijan `X-Workshop-Id` es deliberado y es lo que demuestra RF-502: el cliente pertenece a la empresa, y su ausencia en el contrato es la evidencia del alcance por nivel. El correo es único por empresa y no colisiona entre empresas distintas (RF-503): el duplicado dentro de la misma empresa devuelve `409 client.duplicate_email`.

### 3.6 Inventario — nivel sucursal

| Método | Ruta | Contexto | Rol | Requisito | Resultado |
|---|---|---|---|---|---|
| `GET` | `/api/inventory/parts` | Org+Suc | Miembro | RF-602, RF-607 | Repuestos **de la sucursal activa**; con `lowStock`, solo los que están en o bajo el mínimo |
| `POST` | `/api/inventory/parts` | Org+Suc | Owner, Receptionist | RF-601, RF-603 | Registra un repuesto en la sucursal activa |
| `GET` | `/api/inventory/parts/{partId}` | Org+Suc | Miembro | RF-602 | Datos del repuesto |
| `PATCH` | `/api/inventory/parts/{partId}` | Org+Suc | Owner, Receptionist | RF-601 | Modifica el catálogo del repuesto, **nunca su existencia** |
| `GET` | `/api/inventory/parts/{partId}/movements` | Org+Suc | Miembro | RF-604 | Historial de movimientos del repuesto |
| `POST` | `/api/inventory/parts/{partId}/movements` | Org+Suc | Miembro | RF-604, RF-605, RF-606 | Registra un movimiento y recalcula la existencia, **en una sola transacción** (ADR-007) |
| `POST` | `/api/inventory/parts/{partId}/transfer` | Org+Suc | Owner | RF-608 | Transfiere existencias a otra sucursal de la misma empresa |
...
- Tipos registrables directamente: `compra`, `venta`, `ajuste`, `devolucion`, `merma`. El tipo `transferencia` **no se acepta** en esta operación: lo genera la transferencia entre sucursales (RF-604).
...
- La transferencia genera dos movimientos vinculados —salida en origen, entrada en destino— en una sola transacción, y ambas sucursales deben pertenecer a la misma empresa (RF-608, ADR-007).
- El número de parte es único por sucursal; el duplicado devuelve `409 inventory.duplicate_part_number` (RF-603).

### 3.7 Auditoría

| Método | Ruta | Contexto | Rol | Requisito | Resultado |
|---|---|---|---|---|---|
| `GET` | `/api/audit` | Org | **Owner** | RF-703, RF-704 | Registro de acciones críticas de la empresa activa, con autor, acción y fecha |
...
| `organization.access_denied` | `403` | Sin membresía activa en la empresa indicada |
| `organization.insufficient_permissions` | `403` | El rol no habilita la operación sobre la empresa |
| `organization.not_found` | `404` | La empresa no existe o el solicitante no es miembro |
| `workshop.missing_active_workshop` | `400` | Falta `X-Workshop-Id` |
| `workshop.not_found` | `404` | La sucursal no existe o no pertenece a la empresa activa |
| `workshop.duplicate_name` | `409` | Ya existe una sucursal con ese nombre en la empresa (RF-301) |
| `workshop.insufficient_permissions` | `403` | Solo el `Owner` administra sucursales |
| `member.not_found` | `404` | No hay cuenta con ese correo, o no es miembro de la empresa (RF-401) |
...
| `client.not_found` | `404` | El cliente no existe o pertenece a otra empresa |
| `client.duplicate_email` | `409` | Correo repetido dentro de la empresa (RF-503) |
| `client.insufficient_permissions` | `403` | El rol no permite crear ni modificar clientes (RF-505) |
| `inventory.part_not_found` | `404` | El repuesto no existe o pertenece a otra sucursal |
| `inventory.duplicate_part_number` | `409` | Número de parte repetido en la sucursal (RF-603) |
...
| `inventory.cross_organization_transfer` | `403` | Destino de transferencia fuera de la empresa (RF-608) |
| `inventory.same_workshop_transfer` | `400` | Origen y destino son la misma sucursal |
...
|---|---|---|
| Contexto de una empresa donde no hay membresía | `403 organization.access_denied` | El solicitante declaró operar sobre esa empresa; negarle el acceso no revela nada que no haya afirmado él mismo |
| Recurso de **otra** empresa, estando autenticado en la propia | `404` del módulo correspondiente | Un `403` confirmaría que ese identificador existe en alguna parte. La respuesta debe ser indistinguible de la de un recurso inexistente |
| Sucursal de otra empresa en `X-Workshop-Id` | `404 workshop.not_found` | Mismo motivo |
| Miembro activo, rol insuficiente | `403 <modulo>.insufficient_permissions` | El recurso está dentro de su empresa: su existencia no es información privilegiada, solo la operación lo es |
...
Las entidades de los módulos fuera del alcance (RF-800) se incorporarán siguiendo estas mismas reglas: nivel empresa o nivel sucursal según el criterio del [Glosario](01-glosario.md), contexto por cabecera y códigos `modulo.razon`. El contrato está diseñado para admitirlas sin modificar lo ya especificado.
