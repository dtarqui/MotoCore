-- MotoCore — jerarquia empresa -> sucursales (ADR-006)
--
-- Añade el segundo nivel del modelo multitenant. La empresa (organizations)
-- sigue siendo el UNICO limite de aislamiento; la sucursal (workshops) indica
...
-- que las de nivel empresa: un solo criterio en todo el esquema, que es lo que
...
create table if not exists public.workshops (
  ...
  -- RF-305: la baja es logica; el historial de la sucursal se conserva.
  unique (organization_id, name)
);
create index if not exists workshops_org_id_idx on public.workshops (organization_id);

-- Asignacion operativa de un miembro a una sucursal (RF-304). NO otorga ni
...
-- ------------------------------------------------------------------
-- Helper: la organizacion a la que pertenece una sucursal.
...
-- workshops: cualquier miembro de la empresa ve todas sus sucursales
-- (la sucursal no es frontera de seguridad); solo el Owner las administra.
...
-- workshop_assignments: se resuelve la empresa a traves de la sucursal.
